                                ...
                                <div class="card-item card-item-list">
                                    <div class="card-image">
                                        <a href="{{route('front.getStore',$store->id)}}" class="d-block">
                                            <img title="{{$store->name}}" src="{{asset('assets/front')}}/images/img-loading.png" style="height: 370px" data-src="{{$store->cover}}" class="card__img lazy" alt="">
                                        ...
                                        <ul class="info-list padding-top-20px">
                                            @if ($store->email)
                                                <li><span class="la la-link icon"></span>
                                                    <a target="_blank" href="{{$store->email}}"> {{$store->email}}</a>
                                                </li>
                                            @endif

                                            @if ($store->whatsapp)
                                                <li><span class="la la-whatsapp icon"></span>
                                                    <a href="https://api.whatsapp.com/send?phone= {{$store->whatsapp}}" class="float" target="_blank">
                                                        {{$store->whatsapp}}
                        ...
                        <div class="sidebar-widget">
                            ...
                            <form action="{{route('front.search')}}" class="form-box" method="get">
                                ...
                                <div class="form-group">
                                    ...
                                    <input value="{{request('search_text')}}" name="search_text" class="form-control" type="search" placeholder="عما تبحث؟">
                                </div>
                                ...
                                <div class="form-group user-chosen-select-container">
                                    <select name="category_id" class="user-chosen-select">
                                        ...
                                        @foreach($categories as $cat)
                                            <option {{request('category_id') ==$cat->id ? 'selected':''}} value="{{$cat->id}}">{{$cat->name}}</option>
                                        @endforeach
                                ...
                                <div class="btn-box">
                                    ...
                                    <div class="form-group user-chosen-select-container">
                                        <select name="status" class="user-chosen-select">
                                            ...
                                            <option {{request('status') ==='open' ?'selected':'' }} value="open">مفتوح الأن</option>
                                            <option {{request('status') ==='close' ?'selected':'' }} value="close">مغلق</option>
